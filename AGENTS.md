...
**Listing URLs & SEO:** For routes that use [nuqs](https://nuqs.dev) query state (`query`, `cursor`, `direction`, `limit`), canonical URLs are built with `createLoader` + `createSerializer` from `nuqs/server` (see `src/features/models/listing/listing-canonical.ts`), matching nuqs v2 `clearOnDefault` (true by default; omit default query keys). Wire them in `generateMetadata` as `alternates.canonical` per Next.js [`generateMetadata`](node_modules/next/dist/docs/01-app/03-api-reference/04-functions/generate-metadata.md) / [`alternates`](node_modules/next/dist/docs/01-app/03-api-reference/04-functions/generate-metadata.md#alternates). nuqs docs: [SEO](https://nuqs.dev/docs/seo), [clearOnDefault](https://nuqs.dev/docs/options#clear-on-default).
